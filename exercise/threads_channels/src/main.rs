...
fn main() {
    ...
    let (tx_new, rx_new) = channel::unbounded();
    let rx2_new = rx_new.clone();

    let handle_one = thread::spawn(move || {
        while let Ok(msg) = rx_new.recv() {
            sleep_ms(1);
            println!("Handle One: {msg}");
        }
    });
    let handle_two = thread::spawn(move || {
        while let Ok(msg) = rx2_new.recv() {
            sleep_ms(1);
            println!("Handle Two: {msg}");
        }
    });

    for letter in &["a", "b", "c", "d", "e", "f", "g"] {
        let _ = tx_new.send(letter);
    }
